...
BANDUNG_DISTRICTS = {
    "andir": [r"\bandir\b", r"\bkebon[\s\-]?jeruk\b", r"\bciroyom\b", r"\bdungus[\s\-]?cariang\b", r"\bgaruda\b", r"\bmaleber\b", r"\bcampaka\b"],
    "antapani": [r"\bantapani\b", r"\bantapani[\s\-]?kidul\b", r"\bantapani[\s\-]?tengah\b", r"\bantapani[\s\-]?wetan\b"],
    "arcamanik": [r"\barcamanik\b", r"\bcisaranten[\s\-]?bina[\s\-]?harapan\b", r"\bcisaranten[\s\-]?endah\b", r"\bcisaranten[\s\-]?kulon\b", r"\bsukamiskin\b"],
    "astana anyar": [r"\bastana[\s\-]?anyar\b", r"\bcibadak\b", r"\bkarang[\s\-]?anyar\b", r"\bnyengseret\b", r"\bpanjunan\b", r"\bpelindung[\s\-]?hewan\b"],
    "babakan ciparay": [r"\bbabakan[\s\-]?ciparay\b", r"\bbabakan\b", r"\bcirangrang\b", r"\bmargahayu[\s\-]?utara\b", r"\bmargasuka\b", r"\bsukahaji\b"],
    "bandung kidul": [r"\bbandung[\s\-]?kidul\b", r"\bbatununggal\b", r"\bkujangsari\b", r"\bmengger\b", r"\bwates\b"],
    "bandung kulon": [r"\bbandung[\s\-]?kulon\b", r"\bcaringin\b", r"\bcibuntu\b", r"\bcigondewah[\s\-]?kaler\b", r"\bcigondewah[\s\-]?kidul\b", r"\bcigondewah[\s\-]?rahayu\b", r"\bwarung[\s\-]?muncang\b", r"\bgempol[\s\-]?sari\b"],
    "bandung wetan": [r"\bbandung[\s\-]?wetan\b", r"\bcihapit\b", r"\bcitarum\b", r"\btamansari\b"],
    "batununggal": [r"\bbatununggal\b", r"\bbinong\b", r"\bgumuruh\b", r"\bkebon[\s\-]?waru\b", r"\bkacapiring\b", r"\bkebongedang\b", r"\bmaleer\b", r"\bsamoja\b", r"\bcibangkong\b"],
    "bojongloa kaler": [r"\bbojongloa[\s\-]?kaler\b", r"\bbabakan[\s\-]?asih\b", r"\bbabakan[\s\-]?tarogong\b", r"\bjamika\b", r"\bkopo\b", r"\bsuka[\s\-]?asih\b"],
    "bojongloa kidul": [r"\bbojongloa[\s\-]?kidul\b", r"\bcibaduyut\b", r"\bkebon[\s\-]?lega\b", r"\bmekarwangi\b", r"\bsitusaeur\b"],
    "buahbatu": [r"\bbuah[\s\-]?batu\b", r"\bcijawura\b", r"\bmarga[\s\-]?sari\b", r"\bsekejati\b"],
    "cibeunying kaler": [r"\bcibeunying[\s\-]?kaler\b", r"\bcigadung\b", r"\bcihaurgeulis\b", r"\bneglasari\b", r"\bsukaluyu\b"],
    "cibeunying kidul": [r"\bcibeunying[\s\-]?kidul\b", r"\bcicadas\b", r"\bcikutra\b", r"\bpadasuka\b", r"\bsukamaju\b", r"\bsukapada\b"],
    "cinambo": [r"\bcinambo\b", r"\bbabakan[\s\-]?penghulu\b", r"\bcisaranten[\s\-]?wetan\b", r"\bpakemitan\b", r"\bsukamulya\b"],
    "coblong": [r"\bcoblong\b", r"\bcipaganti\b", r"\bdago\b", r"\blebak[\s\-]?gede\b", r"\blebak[\s\-]?siliwangi\b", r"\bsadang[\s\-]?serang\b", r"\bsekeloa\b"],
    "gedebage": [r"\bgedebage\b", r"\bcimincrang\b", r"\bcisaranten[\s\-]?kidul\b", r"\brancabolang\b", r"\brancanumpang\b"],
    "kiaracondong": [r"\bkiaracondong\b", r"\bbabakan[\s\-]?surabaya\b", r"\bcicaheum\b", r"\bkebon[\s\-]?jayanti\b", r"\bkebon[\s\-]?kangkung\b", r"\bsukapura\b"],
    "lengkong": [r"\blengkong\b", r"\bburangrang\b", r"\bcijagra\b", r"\bcikawao\b", r"\blingkar[\s\-]?selatan\b", r"\bmalabar\b", r"\bpaledang\b", r"\bturangga\b"],
    "mandalajati": [r"\bmandalajati\b", r"\bjati[\s\-]?handap\b", r"\bkarang[\s\-]?pamulang\b", r"\bpasir[\s\-]?impun\b", r"\bsindang[\s\-]?jaya\b"],
    "panyileukan": [r"\bpanyileukan\b", r"\bcibiruwetan\b", r"\bcipadung[\s\-]?kidul\b", r"\bcipadung[\s\-]?kulon\b", r"\bcipadung[\s\-]?wetan\b", r"\bmekarsari\b"],
    "rancasari": [r"\brancasari\b", r"\bcipamokolan\b", r"\bderwati\b", r"\bmanjahlega\b", r"\bmekarjaya\b"],
    "regol": [r"\bregol\b", r"\bancol\b", r"\bbalonggede\b", r"\bcisereuh\b", r"\bcigereleng\b", r"\bpungkur\b", r"\bpasirluyu\b"],
    "sukajadi": [r"\bsukajadi\b", r"\bcipedes\b", r"\bpasteur\b", r"\bsukabungah\b", r"\bsukagalih\b", r"\bsukawarna\b"],
    "sukasari": [r"\bsukasari\b", r"\bgegerkalong\b", r"\bisola\b", r">\sarijadi\b", r">\sukarasa\b"],
    "sumur bandung": [r"\bsumur[\s\-]?bandung\b", r"\bbabakan[\s\-]?ciamis\b", r"\bbraga\b", r"\bkebon[\s\-]?pisang\b", r"\bmerdeka\b"],
    "ujung berung": [r"\bujung[\s\-]?berung\b", r"\bcigending\b", r"\bpasanggrahan\b", r"\bpasir[\s\-]?endah\b", r"\bpasir[\s\-]?wangi\b", r"\bpasir[\s\-]?jati\b"],
}
